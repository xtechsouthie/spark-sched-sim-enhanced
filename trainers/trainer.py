...
class Trainer(ABC):
    ...
    def train(self) -> None:
        ...
        print(f"DEBUG: Training for {self.num_iterations} iterations", flush=True)
        ...
        for i in range(self.num_iterations):
            ...
            if exception:
                print("DEBUG: Exception occurred in receiving results from worker, breaking training loop", flush=True)
                break
            ...
            # avg_num_jobs tells the congestion of the system, the lower, the better.
            ...
            if (i + 1) % self.checkpointing_freq == 0:
                print(f"DEBUG: Checkpointing at iteration {i+1}", flush=True)
        ...
        if exception:
            print("DEBUG: Raising exception that occurred during training (train() method)", flush=True)
            raise exception
    ...
    def evaluate_both_baselines(self,env_cfg):
        import spark_sched_sim  # Import to register the environment
        original_use_critic = self.use_critic

        eval_env = gym.make("spark_sched_sim:SparkSchedSimEnv-v0", env_cfg=env_cfg)

        if hasattr(self.scheduler, 'env_wrapper_cls') and self.scheduler.env_wrapper_cls:
            eval_env = self.scheduler.env_wrapper_cls(eval_env)

        results = {}

        # Evaluation 1: without critic (but this doesn't actually change policy behavior)
        ...
        for _ in range(5):
            ...
            while not done:
                action, _ = self.scheduler.schedule(obs)
                obs, reward, terminated, truncated, info = eval_env.step(action)
                done = terminated or truncated
                ep_reward += reward

            job_durations.append(eval_env.unwrapped.avg_job_duration) # it is already converted to sec in spark_sched_sim.py.
            rewards.append(ep_reward)

        results['original_baseline'] = {
        ...
        if hasattr(self, 'use_critic') and original_use_critic:
            self.use_critic = True
            self.scheduler.eval()
            
            # Reset random seed again for consistent comparison
            torch.manual_seed(self.seed)
            random.seed(self.seed)
            np.random.seed(self.seed)
            ...
            for _ in range(5):
                ...
                job_durations.append(eval_env.unwrapped.avg_job_duration)
                rewards.append(ep_reward)

            results['critic_baseline'] = {
            'avg_job_duration': np.mean(job_durations),
            'std_job_duration': np.std(job_durations),
            'avg_reward': np.mean(rewards),
            'std_reward': np.std(rewards)
        ...
        if 'critic_baseline' in results and 'original_baseline' in results:
            original_duration = results['original_baseline']['avg_job_duration']
            critic_duration = results['critic_baseline']['avg_job_duration']
            improvement = (original_duration - critic_duration) / original_duration * 100

            results['comparison'] = {
                'absolute_improvement': original_duration - critic_duration,
                'percentage_improvement': improvement,
                'better_method': 'critic' if improvement > 0 else 'original'
            }

        self.use_critic = original_use_critic
    ...
    def _setup(self) -> None:
        ...
        # Comment out stdout redirection for debugging
    ...
    def train_and_compare_models(self, train_cfg: dict, agent_cfg: dict, env_cfg: dict) -> dict:
        print('Training and comparing two models: with and without critic')

        original_use_critic = train_cfg.get('use_critic', False)
        ...
        cfg_without_critic['trainer']['use_critic'] = False
        cfg_without_critic['trainer']['artifacts_dir'] = f'{original_artifacts_dir}/no_critic'

        trainer_without_critic = make_trainer(cfg_without_critic)
        trainer_without_critic.train()

        without_critic_model_path = f'{cfg_without_critic["trainer"]["artifacts_dir"]}/checkpoints/final_no_critic.pt'
        ...
        cfg_with_critic['trainer']['use_critic'] = True
        cfg_with_critic['trainer']['artifacts_dir'] = f'{original_artifacts_dir}/with_critic'

        trainer_with_critic = make_trainer(cfg_with_critic)
        trainer_with_critic.train()

        with_critic_model_path = f'{cfg_with_critic["trainer"]["artifacts_dir"]}/checkpoints/final_with_critic.pt'
